...
legLength  = 1;           % Length of the leg
mass       = 1;           % Mass of the human, concentrated at a single point
Fmax       = 200*mass*g;
stepLength = 0.5;         % The target step length - Unused
Speed      = 1.2;

%% Pack parameters
params.g = g; params.L0 = legLength; params.m = mass; params.stepLength = stepLength;
params.Fmax = Fmax; params.Speed = Speed;

%% Initial conditions
x0    = 0;       % Start at mid-stance
vx0   = 0.01;    % with some forward velocity
y0    = 0.7;     % Start somewhere that doesn't break leg length constraints
vy0   = 0;       % No velocity along the leg                          
footX = 0;       % Initial foot position forward     
footY = 0;       % Initial foot position vertical

F0   = mass*g;   % Force along the leg
delF = 0.01;     % Change in force in this simulation segment
delT = 0.01;     % Duration of the simulation segment
...
subplot(3,1,3)
Ldotdot = F*legLength/mass - g*y + vx.^2 + vy.^2;
