% 2-D telescoping leg walker
% The walker has a point-mass and point-feet. In each step, the mass
% is driven by gravity and the telescoping force along the leg. 
% Push-off and heel-strike happen simultaneously such that the 
% double-support period is instantaneous.
close all;
...
% Control input at the starting point
Fstart = 8;

% Multiple shooting params
numShots = 40;
params.delT = 0.88/numShots; 
params.delF = 8/numShots;

% Starting time
t0 = 0;                   

% Starting state
state0   = [x0; y0; vx0; vy0; Fstart; footX; footY;];
...
for currShot = 1:numShots 
    ...
    tSpan = [0, params.delT]; %linspace(0,params.delT,100);

    % simulate a step
    options = odeset('reltol',1e-9,'abstol',1e-9);
    [tListOut,stateListOut] = ode45(ODE_walk,tSpan,state0,options);

    stateStore = [stateStore; stateListOut(end,:)]; % leaving out the last point to avoid repetition
    timeStore = [timeStore; t0 + tListOut(end)];
